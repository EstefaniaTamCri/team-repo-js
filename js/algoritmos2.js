...
const A2E7 = () => {
  ...
  while (attempts > 0) {
    ...
    if (isNaN(answer) || answer > max || answer < min) {
      ...
      //Si la respuesta es igual al número
      console.log("You win!");
      break;
    } else if (attempts > 1) {
      //Si la respuesta es distinta al número
...
function A2E8() {
  ...
  let long = parseInt(
    prompt("Introduce la longitud de la contraseña (entre 8 y 16 caracteres):")
  );
  ...
  if (isNaN(long)) {
    ...
    long = parseInt(
      prompt(
        "Introduce la longitud de la contraseña (entre 8 y 16 caracteres):"
      )
    );
  }
  ...
  if (!isNaN(long)) {
    ...
    let caract = confirm("¿Quieres incluir símbolos?");
    ...
    if (caract) {
      caracteres +=
        generarCaracteres(33, 47) +
        generarCaracteres(58, 64) +
        generarCaracteres(91, 96) +
        generarCaracteres(123, 126); // caracteres especiales
    }
